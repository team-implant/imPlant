...
void static wifi_clear_databuffer_and_index() {
    for (uint16_t i = 0; i < WIFI_DATABUFFERSIZE; i++) wifi_dataBuffer[i] = 0;
...
WIFI_ERROR_MESSAGE_t wifi_command_AT() { return wifi_command("AT", 1); }
...
WIFI_ERROR_MESSAGE_t wifi_command_create_TCP_connection(
    char *IP, uint16_t port, WIFI_TCP_Callback_t callback_when_message_received,
...
WIFI_ERROR_MESSAGE_t wifi_command_TCP_transmit(uint8_t *data, uint16_t length) {
    ...
    WIFI_ERROR_MESSAGE_t errorMessage = wifi_command(sendbuffer, 20);
    if (errorMessage != WIFI_OK) return errorMessage;
